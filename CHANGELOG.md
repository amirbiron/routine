...
**פירוט:** סקריפט המיגרציה נכשל ב-`ER_DUP_ENTRY` כאשר ניסה להוסיף אינדקס ייחודי על `userId` בטבלת `reminderSettings`, כי כבר היו שורות כפולות. הפתרון: (1) הוספת שלב ניקוי כפילויות לפני יצירת האינדקס — שומר את הרשומה האחרונה לכל userId ומוחק את הישנות. (2) הוספת `ER_DUP_ENTRY` לרשימת השגיאות הצפויות כ-fallback נוסף.
