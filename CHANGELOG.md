...
### תיקון: כשל בבילד עקב כפילויות בטבלת reminderSettings
**קבצים:** `scripts/migrate.mjs`
**פירוט:** סקריפט המיגרציה נכשל ב-`ER_DUP_ENTRY` כאשר ניסה להוסיף אינדקס ייחודי על `userId` בטבלת `reminderSettings`, כי כבר היו שורות כפולות. הפתרון: (1) הוספת שלב ניקוי כפילויות לפני יצירת האינדקס — שומר את הרשומה האחרונה לכל userId ומוחק את הישנות. (2) דיכוי `ER_DUP_ENTRY` מוגבל רק ל-statement של reminderSettings ולא גלובלי — כך ש-unique constraints אחרים (כמו email בטבלת users) ימשיכו לכשול בצורה רועשת אם יש כפילויות.
